...
local _LOGGER = logging.get_logger("treemotion._commands.copy_logs.runner")

local M = {}

--- Modify the user's system clipboard with `result`.
---
---@param result treemotion.ReadFileResult The file path + its contents that we read.
---
...
---@class treemotion.ReadFileResult
...
---@param callback fun(result: treemotion.ReadFileResult): nil Call this once `path` is read.
---@private
---
