...
- `w` / `e` / `b` / `ge` move one treesitter leaf at a time (an identifier, a
  ...
  Rust's `///` -- or a bare `-`/`_` run with no identifier beside it, like
  ...
  real letter or digit, e.g. `hello-world`). See `commands.motion.subword.code`
  / `.prose` under `Configuration` to control this per context.
